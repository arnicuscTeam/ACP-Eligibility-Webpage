...
st.write("This tool uses microdata from the American Community Survey (ACS) 1-year 2021 [estimates]"
         ...
         "and the geographic correspondence [files](https://mcdc.missouri.edu/applications/geocorr.html) developed by "
         "the Missouri Census Data Center to estimate the number of ACP eligible households using different eligibility "
         "criteria for different geographic units. To begin, select the desired geographic level of analysis in the "
         "dropdown menu at the top (state, county, PUMA, zipcode, metro area or Congressional District). Next, use the "
         "slider to select an income eligibility threshold between 200% (the current income threshold) and 120% of the "
         "Federal Poverty Line. There are four ACS variables that capture information about additional ACP qualification "
         "criteria for households (see table below for description of these ACS variables). Please check or uncheck the "
         "boxes to obtain estimates that include/exclude these programs. Finally, select the demographic characteristics "
         "to obtain estimates for specific sub-populations (this is optional, leave blank for entire population). Then "
         "click “Submit” and download the dataset corresponding to these eligibility criteria. ")
...
table_html = "<table style='border-collapse: collapse; width: 100%; border: 1px solid #ccc; background-color: transparent;'>"
table_html += "<tr style='background-color: transparent;'>"
table_html += "<th style='border: 1px solid #ccc; padding: 8px; text-align: left; background-color: transparent;'>ACS Variable Description</th>"
table_html += "<th style='border: 1px solid #ccc; padding: 8px; text-align: left; background-color: transparent;'>Variable name</th>"
table_html += "</tr>"
...
has_hins4 = int(st.checkbox('Receives Medicaid, Medical Assistance, or any kind of government-assistance plan (variable: HINS4)', value=True))
...
if bottom_text:
    ...
    st.markdown("<span style='font-size: 14px;'>This tool was developed by a team at the University of Southern California led by [Prof. Hernan Galperin](https://annenberg.usc.edu/faculty/hernan-galperin) and [Prof. François Bar](https://annenberg.usc.edu/faculty/fran%C3%A7ois-bar), with research assistance from Angel Chavez-Penate. The Pew Research Trust has generously provided funding for this project.</span>", unsafe_allow_html=True)
